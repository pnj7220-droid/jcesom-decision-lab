...
## Facilitator mode — viewing all participant scores

The app stores every completed scenario in a local SQLite database (`scores.db`) on the Streamlit Cloud instance. Only the facilitator can see other participants' scores; staff who use the app only see their own results.

**To view scores:**

1. Append `?facilitator=1` to your app URL — for example:
   `https://jcesom-decision-lab.streamlit.app/?facilitator=1`
2. Enter the facilitator password.
3. You'll see total participants, total completed scenarios, average score, a sortable table of every result, and a CSV download.

The facilitator password is set directly in the code (in `app.py`, near the top, look for `FACILITATOR_PASSWORD`). To change it, edit that line, commit to GitHub, and Streamlit will redeploy automatically.

**Storage note:** the SQLite database persists across user sessions but resets every time the app redeploys (e.g., when you push a code change). For a workshop or live training, download the CSV before pushing any new changes if you want to keep the records.

---

## Deploy to Streamlit Community Cloud (free)

1. Create a new GitHub repo and push `app.py`, `marshall_seal.png`, `requirements.txt`, and `README.md`.
